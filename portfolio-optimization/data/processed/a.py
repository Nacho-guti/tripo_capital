...
class PortfolioOptimiser:
    ...
    def summary(self, weights: np.ndarray, threshold: float = 1e-6) -> pd.DataFrame:
        """
        Returns a DataFrame of selected assets with weights above threshold,
        including expected return, volatility, and individual Sharpe ratio.
        """
        df = pd.DataFrame({
            'Ticker': self.price_df.columns,
            'Weight': weights,
            'ExpReturn': self.mean_returns.values,
            'Volatility': np.sqrt(np.diag(self.cov_matrix.values)),
        })
        df['Sharpe'] = (df['ExpReturn'] - self.risk_free_rate) / df['Volatility']
        # Filter by threshold and sort descending by weight
        df_selected = df[df['Weight'] > threshold].copy()
        return df_selected.sort_values(by='Weight', ascending=False).reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    # Output results: overall performance
    perf = opt._portfolio_performance(weights)
    print(f"Portfolio Return: {perf['return']:.2%}")
    print(f"Portfolio Volatility: {perf['volatility']:.2%}")
    print(f"Portfolio Sharpe: {perf['sharpe']:.2f}\n")

    # Detailed summary of selected assets
    summary_df = opt.summary(weights)
    print("Selected Assets Summary:")
    ...
    output_path = Path("portfolio-optimization/results/selected_assets_summary.xlsx")
    summary_df.to_excel(output_path, index=False)
    print(f"\nSummary exported to {output_path.resolve()}")